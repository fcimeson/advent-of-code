...
def play(game, called_number):
    N = len(game['board'])
    while N > len(game['row_count']): game['row_count'].append(0)
    while N > len(game['col_count']): game['col_count'].append(0)
        ...
        for line in f.readlines():
            if state == 'parse_numbers_to_call':
                ...
                if re.match(r'^\s*$', line):
                    ...
                    games[-1]['board'].append([int(x) for x in re.findall(r'\d+', line)])
    ...
    for called_number in numbers_to_call:
        ...
        for i, game in enumerate(games):
            if play(game, called_number):
                ...
                print(f"Player {i} won with a score of {game_score}. Called numbers {called_numbers}.")
                winner = True
                break
        if winner:
            break
