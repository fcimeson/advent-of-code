...
if __name__ == "__main__":
    ...
    with open(args.input, 'r') as f:
        for line in f.readlines():
            if re.match(input_rx, line):
                s = line.split('|')
                data.append({'signals':re.findall(RE_DIGIT, s[0]), 'output':re.findall(RE_DIGIT, s[1])})
            else:
    ...
    for x in data:
        ...
        if args.debug:
            print(s)
    print(f"The number of 1s, 4s, 7s, and 8s is {number_of_digits[1]}, {number_of_digits[4]}, {number_of_digits[7]}, {number_of_digits[8]} respectively ({number_of_digits[1] + number_of_digits[4] + number_of_digits[7] + number_of_digits[8]} in total)")
